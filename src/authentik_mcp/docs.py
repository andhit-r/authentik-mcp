...
FastMCP berjalan di atas Starlette dan mengekspos protokol MCP pada satu
endpoint HTTP. Modul ini menambahkan rute dokumentasi:

* ``GET /openapi.json`` — dokumen OpenAPI 3.1 yang dihasilkan otomatis dari
  daftar tool MCP yang terdaftar (nama, deskripsi, dan skema parameter).
* ``GET /docs``        — halaman Swagger UI yang membaca ``/openapi.json``.
* ``GET /health``      — health check sederhana.

Tool MCP dipanggil melalui protokol MCP (bukan REST murni), sehingga dokumen
OpenAPI di sini bersifat **deskriptif**: ia mendokumentasikan setiap tool
beserta skema inputnya agar mudah ditinjau lewat Swagger UI.
"""
...
# Path endpoint MCP yang sebenarnya (tempat tool dipanggil via protokol MCP).
_MCP_PATH = "/mcp/"
...
<head>
  <meta charset="UTF-8" />
  <title>authentik-mcp — API Docs</title>
...
async def build_openapi_spec(mcp: FastMCP, version: str) -> dict[str, Any]:
    ...
    Returns:
        Dict dokumen OpenAPI 3.1 yang siap di-serialisasi ke JSON.
    """
    ...
    return {
        ...
        "info": {
            ...
            "version": version,
            "description": (
                "MCP server untuk mengelola Authentik. Endpoint protokol MCP "
...
def register_docs_routes(mcp: FastMCP, version: str) -> None:
    ...
    async def health(_request: Request) -> JSONResponse:
        ...
        return JSONResponse({"status": "ok", "service": "authentik-mcp", "version": version})
